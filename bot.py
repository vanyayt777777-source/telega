...
async def load_more_chats(client: Client, message: Message):
    ...
    # Определяем тип выбора (рассылка или реакции)
        ...
        if message.text.isdigit():
            ...
            if 1 <= chat_number <= len(chats):
                ...
                if len(selected) > max_chats:
                    ...
                    await message.reply_text(
                        f"❌ **Нельзя выбрать больше {max_chats} чатов!**"
                    )
                else:
...
async def start_mass_reactions(client: Client, message: Message, user_id: int, temp_data: dict):
    ...
    # Создаем запись о реакциях в БД
    ...
    # Запускаем фоновую задачу для отслеживания сообщений
    ...
    while active_reaction_tasks.get(user_id, False):
        try:
            ...
            async for message in user_client.get_chat_history(chat_id, limit=5):
                ...
                # Проверяем, не ставили ли уже реакцию
                if message.id > last_message_id:
                    try:
                        ...
                        # Уведомляем пользователя (не чаще чем раз в 5 сообщений)
                        if message.id % 5 == 0:
                ...
                break  # Проверяем только самое новое сообщение
