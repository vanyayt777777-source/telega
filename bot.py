...
def get_creation_type_keyboard():
    return ReplyKeyboardMarkup(
        [
...
@app.on_message(filters.text & filters.regex("^📊 Статистика$"))
async def statistics_menu(client: Client, message: Message):
    ...
    text = (
        ...
        f"📱 **Аккаунтов:** {len(accounts)}/{db.get_max_accounts(user_id)}\n"
        f"📨 **Сообщений отправлено:** {user_data['messages_sent']}\n"
        f"📢 **Создано каналов:** {len(channels)}\n"
        f"👥 **Создано групп:** {len(groups)}\n"
        f"💎 **Подписка:** {'✅ Активна' if db.check_subscription(user_id) else '❌ Бесплатная'}\n\n"
        f"📢 **Наш канал:** @VestSoftTG\n"
        f"💬 **Поддержка:** @VestSoftSupport"
    )
...
@app.on_message(filters.text & ~filters.regex("^(📱 Менеджер аккаунтов|⚙️ Функции|📊 Статистика|👤 Мой профиль|📢 Наш канал|➕ Добавить аккаунт|📋 Список аккаунтов|🔑 Выбрать активный аккаунт|❌ Удалить аккаунт|🔄 Обновить данные|◀️ Назад в главное меню|◀️ Назад|❌ Отмена|📨 Рассылка|📢 Создание каналов|👥 Создание групп|🤖 Проверка спамблока|📥 Загрузить ещё чаты|✅ Завершить выбор и продолжить|📝 Ввести сообщения|✅ Запустить рассылку|✅ Да|❌ Нет|/done)$") & ~filters.regex("^🔄 Режим:.*$") & ~filters.regex("^⚡ Автоподписка:.*$") & ~filters.regex("^[⬜✅] .* - .*$") & ~filters.regex("^✅ Начать проверку$"))
async def handle_text_input(client: Client, message: Message):
    ...
    elif step == "creating_channel" or step == "creating_group":
        # Сохраняем название
...
async def execute_creation(client: Client, message: Message, user_id: int, temp_data: dict):
    ...
    account_id = temp_data.get('account_id')
    creation_type = temp_data.get('creation_type')  # 'channel' или 'group'
    ...
    for i in range(1, count + 1):
        try:
            ...
            # Отправляем прогресс каждые 5 созданий
            ...
            # Пробуем снова для этого же индекса
            i -= 1
            ...
            for task in pending_tasks:
                try:
                    ...
                    if task['button_data'] and task['button_data'].get('type') == 'callback':
                        ...
                        # Для URL кнопок просто уведомляем
                        await client.send_message(
                            user_id,
