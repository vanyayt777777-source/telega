...
async def mailing_start(client: Client, message: Message):
    ...
    try:
        # Загружаем диалоги (только группы и каналы)
        chats = []
        async for dialog in user_client.get_dialogs():
            if dialog.chat.type in [ChatType.GROUP, ChatType.SUPERGROUP, ChatType.CHANNEL]:
                ...
                # Ограничим количество чатов для удобства
                if len(chats) >= 50:
                    break
...
async def show_next_chat_for_selection(client: Client, message: Message, user_id: int, account_id: int):
    ...
    await message.reply_text(
        f"📢 *Выбор чатов ({len(selected)}/{MAX_CHATS_PER_MAILING})*\n\n"
...
async def select_current_chat(client: Client, message: Message):
    ...
    await message.reply_text(f"✅ Чат '{chat['chat_title']}' добавлен в рассылку!")
...
@app.on_message(filters.text & ~filters.regex("^(📱 Менеджер аккаунтов|⚙️ Функции|👤 Профиль|➕ Добавить аккаунт|📋 Список аккаунтов|🔑 Выбрать активный аккаунт|◀️ Назад в главное меню|◀️ Назад|❌ Отмена|📨 Рассылка|✅ Выбрать этот чат|⏭ Пропустить|✅ Завершить выбор)$"))
