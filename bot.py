...
async def profile_menu(client: Client, message: Message):
    ...
    if has_subscription:
        ...
        text += "Бесплатная\n"
        text += "⚡ При бесплатной подписке к сообщениям добавляется реклама бота\n\n"
...
async def start_mailing(client: Client, message: Message, user_id: int, temp_data: dict):
    ...
    # Если нет подписки, добавляем рекламный текст
    if not has_subscription:
        message_text = message_text + PROMO_TEXT
        await client.send_message(
            user_id,
            "⚡ У вас бесплатная подписка. К сообщению добавлена реклама бота.\n"
