...
from pyrogram.errors import AccessTokenInvalid, UserIsBlocked, ChatAdminRequired
...
def get_spamblock_accounts_keyboard(accounts: list):
    ...
    for acc in accounts:
        ...
        keyboard.append([KeyboardButton(f"{status} {acc['account_name']} - {acc['phone_number']}")])
...
async def spamblock_check_start(client: Client, message: Message):
    ...
    db.save_temp_data(user_id, {
        ...
        "accounts": [{"id": acc["id"], "name": acc["account_name"], 
                     "phone": acc["phone_number"], "selected": False} for acc in accounts]
    })
    ...
    await message.reply_text(
        ...
        reply_markup=get_spamblock_accounts_keyboard(db.get_temp_data(user_id)["accounts"])
    )
...
@app.on_message(filters.text & ~filters.regex("^(📱 Менеджер аккаунтов|⚙️ Функции|👤 Профиль|➕ Добавить аккаунт|📋 Список аккаунтов|🔑 Выбрать активный аккаунт|❌ Удалить аккаунт|◀️ Назад в главное меню|◀️ Назад|❌ Отмена|📨 Рассылка|🤖 Проверка спамблока|📥 Загрузить ещё чаты|✅ Завершить выбор и продолжить|📝 Ввести сообщения|✅ Запустить рассылку)$") & ~filters.regex("^🔄 Режим:.*$") & ~filters.regex("^⚡ Автоподписка:.*$"))
async def handle_text_input(client: Client, message: Message):
    ...
    elif step == "selecting_spamblock_accounts":
        # Проверяем, является ли сообщение выбором аккаунта
...
async def check_spamblock(client: Client, message: Message, user_id: int, selected_accounts: list):
    ...
    for acc_data in selected_accounts:
        account = db.get_user_accounts(user_id)
        account = next((a for a in account if a['id'] == acc_data['id']), None)
...
# Функция для проверки статуса аккаунтов (запускается периодически)
async def check_accounts_status():
    """Периодическая проверка статуса аккаунтов (не заблокированы ли)"""
    while True:
        try:
            ...
            for (user_id,) in users:
                ...
                for account in accounts:
                    ...
                    try:
                        client = await get_user_client(user_id, account)
                        if client:
                            me = await client.get_me()
                            # Аккаунт работает
                    except Exception as e:
                        # Проверяем, не заблокирован ли аккаунт
                        if "USER_DEACTIVATED" in str(e) or "USER_DEACTIVATED_BAN" in str(e):
                            # Аккаунт заблокирован
                            with db.get_connection() as conn:
                                cursor = conn.cursor()
                                cursor.execute("""
                                    UPDATE accounts 
                                    SET is_blocked = 1, block_date = ? 
                                    WHERE id = ?
                            ...
                            try:
                                await app.send_message(
                                    user_id,
                                    f"⚠️ Аккаунт {account['account_name']} ({account['phone_number']}) был заблокирован Telegram!"
                                )
                            except:
                                pass
                            
                            # Останавливаем клиент
                            await stop_user_client(user_id, account['id'])
            ...
            print(f"Ошибка в проверке статуса аккаунтов: {e}")
            await asyncio.sleep(3600)

# Функция для проверки просроченных подписок
            ...
            with db.get_connection() as conn:
                ...
                for (user_id,) in expired_users:
                    ...
                    try:
                        await app.send_message(
                            user_id,
                            "⚠️ Срок вашей подписки истек. "
                            "Вы переведены на бесплатный тариф.\n"
                            "Купите подписку снова в профиле, чтобы продолжить пользоваться расширенными возможностями."
                        )
                    except:
                        pass
...
if __name__ == "__main__":
    ...
    loop.create_task(check_accounts_status())
