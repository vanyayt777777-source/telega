...
async def check_accounts_status():
    ...
    while True:
        try:
            ...
            print("✅ Проверка статуса аккаунтов завершена. Следующая проверка через 1 час.")
...
async def check_expired_subscriptions():
    ...
    while True:
        try:
            ...
            print("✅ Проверка истекших подписок завершена. Следующая проверка через 1 час.")
...
async def start_background_tasks():
    """Запускает фоновые задачи после старта клиента"""
    try:
        # Ждем немного, чтобы клиент точно запустился
...
async def show_chats_page(client: Client, message: Message, user_id: int, account_id: int, offset: int):
    ...
    temp_data['last_message_id'] = sent_msg.id
    db.save_temp_data(user_id, temp_data)
