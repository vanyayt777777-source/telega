...
def get_autoreply_keyboard():
    return ReplyKeyboardMarkup(
        [
            ...
            [KeyboardButton("▶️ Запустить автоответчик")],
...
class Database:
    ...
    def enable_all_auto_reply_rules(self, user_id: int, account_id: int):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE auto_reply_rules SET is_active = 1 WHERE user_id = ? AND account_id = ?",
                (user_id, account_id)
            )
            conn.commit()
...
async def test_proxy(proxy_ip: str, proxy_port: int, proxy_login: str, proxy_password: str) -> bool:
    """Тестирует работоспособность прокси"""
    try:
        # Создаем тестовый клиент с прокси
        proxy_dict = {
            "scheme": "http",
            "hostname": proxy_ip,
            "port": proxy_port,
...
async def add_account_start(client: Client, message: Message):
    ...
    proxy_text = (
        ...
        "После добавления аккаунта вы сможете настроить прокси в меню аккаунтов.\n\n"
...
async def auto_reply_menu(client: Client, message: Message):
    ...
    # Проверяем статус автоответчика
    rules = db.get_all_auto_reply_rules(user_id, active_account['id'])
    active_count = len([r for r in rules if r['is_active']])
    total_count = len(rules)
    
    status_text = f"✅ Активен ({active_count}/{total_count} правил)" if active_count > 0 else "❌ Остановлен"
    
    await message.reply_text(
        f"🤖 **Автоответчик**\n\n"
        f"**Статус:** {status_text}\n\n"
        f"**Доступные действия:**\n"
        f"• ➕ Добавить правило (Триггер | Ответ)\n"
        f"• 📋 Список правил\n"
        f"• ❌ Удалить правило\n"
        f"• ▶️ Запустить автоответчик (активировать все правила)\n"
        f"• ⏹️ Остановить автоответчик (деактивировать все правила)\n\n"
        f"Выберите действие: 👇",
        reply_markup=get_autoreply_keyboard()
    )

@app.on_message(filters.text & filters.regex("^▶️ Запустить автоответчик$"))
async def start_auto_reply(client: Client, message: Message):
    user_id = message.from_user.id
    active_account = db.get_active_account(user_id)
    ...
    # Активируем все правила
    db.enable_all_auto_reply_rules(user_id, active_account['id'])
    
    rules = db.get_all_auto_reply_rules(user_id, active_account['id'])
    
    await message.reply_text(
        f"✅ **Автоответчик запущен!**\n\n"
        f"Активировано правил: {len(rules)}",
        reply_markup=get_autoreply_keyboard()
    )
...
async def list_auto_reply_rules(client: Client, message: Message):
    ...
    for i, rule in enumerate(rules[:10], 1):
        ...
        active_count += 1 if rule['is_active'] else 0
        text += f"{status} {i}. **{rule['trigger_text']}** → {rule['reply_text'][:30]}...\n"
    ...
    text += f"📊 **Всего правил:** {len(rules)} (активных: {active_count})"
...
@app.on_message(filters.text & ~filters.regex("^(📱 Менеджер аккаунтов|⚙️ Функции|👤 Мой профиль|➕ Добавить аккаунт|📋 Список аккаунтов|🔑 Выбрать активный аккаунт|❌ Удалить аккаунт|🌐 Настроить прокси|🔄 Обновить данные|◀️ Назад в главное меню|◀️ Назад|❌ Отмена|📨 Рассылка|📢 Создание каналов|👥 Создание групп|❤️ Масс-реакции|🤖 Автоответчик|🔍 Парсинг пользователей|🤖 Проверка спамблока|➕ Добавить правило|📋 Список правил|❌ Удалить правило|▶️ Запустить автоответчик|⏹️ Остановить автоответчик|🌐 Все чаты|📋 Выбрать чаты|📥 Загрузить ещё чаты|✅ Завершить выбор и продолжить|📝 Ввести сообщения|✅ Запустить рассылку|/done|/stop_reactions|/stopauto|/stopparse)$") & ~filters.regex("^🔄 Режим:.*$") & ~filters.regex("^⚡ Автоподписка:.*$") & ~filters.regex("^[⬜✅] .* - .*$") & ~filters.regex("^✅ Начать проверку$") & ~filters.regex("^👍 Лайк|👎 Дизлайк|❤️ Сердечко|🔥 Огонь|🎉 Праздник$"))
    ...
    if step == "waiting_phone":
        ...
        if len(parts) == 4:
            ...
            try:
                ...
                # Тестируем прокси
                status_msg = await message.reply_text("🔄 **Тестирую прокси...**")
                
                if await test_proxy(ip, port, login, password):
                    # Сохраняем прокси
                    ...
                    await status_msg.edit_text(
                        ...
                        f"🌐 {ip}:{port}\n"
                        f"👤 {login}:{password}\n\n"
                        f"Прокси протестирован и работает.",
                        reply_markup=get_account_manager_keyboard()
                    )
                else:
                    await status_msg.edit_text(
                        "❌ **Прокси не работает!**\n\n"
                        "Проверьте правильность данных или попробуйте другой прокси.",
                        reply_markup=get_cancel_keyboard()
                    )
                    return
        ...
        if "|" in rule_text:
            ...
            if trigger and reply:
                ...
                if scope == 'all':
                    ...
                    all_chats = temp_data.get('all_chats', [])
                    if all_chats:
                        ...
                        await message.reply_text(
                            ...
                            f"🔍 **Триггер:** {trigger}\n"
                            f"💬 **Ответ:** {reply}\n\n"
                            f"▶️ Не забудьте запустить автоответчик!",
                            reply_markup=get_autoreply_keyboard()
                        )
                    else:
                        await message.reply_text(
...
async def parse_chat_users(client: Client, message: Message, user_id: int, account_id: int, chat_link: str):
    """Парсит пользователей из чата и сохраняет в файл"""
    ...
    try:
        ...
        async for member in user_client.get_chat_members(chat_id):
            ...
            if member.user:
                ...
                if count % 50 == 0:
                    await client.send_message(
                        user_id,
                        f"📊 **Прогресс:** спарсено {count} пользователей..."
                    )
        ...
        if count > 0:
            ...
            await client.send_message(
                ...
                "Возможно, у вас нет доступа к списку участников."
            )
...
async def auto_reply_handler(client: Client, message: Message):
    ...
    for (user_id,) in users:
        ...
        for account in accounts:
            ...
            # Получаем активные правила для этого чата
            ...
            for rule in rules:
                if rule['is_active'] and rule['trigger_text'].lower() in message_text.lower():
                    # Получаем клиент для аккаунта
                    user_client = await get_user_client(user_id, account)
                    if not user_client:
                        continue
