...
async def auto_reply_handler(client: Client, message: Message):
    ...
    for (user_id,) in users:
        ...
        for account in accounts:
            ...
            # Получаем правила для этого чата
            rules = db.get_auto_reply_rules(user_id, account['id'], chat_id)
            
            if not rules:
                # Если нет правил для конкретного чата, проверяем правила для всех чатов
                all_rules = db.get_auto_reply_rules(user_id, account['id'])
                rules = [r for r in all_rules if r['chat_id'] == 'all']
