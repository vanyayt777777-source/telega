...
# Инициализация бота (только для бота, API бота не используется для пользовательских аккаунтов)
app = Client(
    "vest_soft_bot",
    bot_token=BOT_TOKEN
)
...
class Database:
    ...
    def add_account(self, user_id: int, api_id: str, api_hash: str, phone_number: str, session_string: str) -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                """INSERT INTO accounts 
                   (user_id, api_id, api_hash, phone_number, session_string, account_name) 
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (user_id, api_id, api_hash, phone_number, session_string, account_name)
            )
    ...
    def save_chats(self, user_id: int, account_id: int, chats: list):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            # Очищаем старые чаты для этого аккаунта
            cursor.execute(
            ...
            for chat in chats:
                cursor.execute(
                    """INSERT INTO chats (user_id, account_id, chat_id, chat_title) 
                       VALUES (?, ?, ?, ?)""",
                    (user_id, account_id, str(chat['id']), chat['title'])
                )
            conn.commit()
    
    def get_chats(self, user_id: int, account_id: int) -> List[Dict]:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
...
async def account_manager_menu(client: Client, message: Message):
    ...
    if accounts:
        text += f"У вас добавлено аккаунтов: {len(accounts)}/{MAX_ACCOUNTS}\n\n"
        for acc in accounts:
...
async def add_account_start(client: Client, message: Message):
    ...
    db.save_temp_data(user_id, {"step": "waiting_api_id"})
    
    await message.reply_text(
        "📝 Для добавления аккаунта мне понадобятся ваши API данные.\n\n"
        "✏️ Введите ваш API ID (число):\n"
        "(Чтобы получить API данные: https://my.telegram.org/apps)",
        reply_markup=get_cancel_keyboard()
    )

@app.on_message(filters.text & filters.regex("^📋 Выбрать аккаунт$"))
async def select_account_menu(client: Client, message: Message):
    user_id = message.from_user.id
    accounts = db.get_user_accounts(user_id)
...
async def mailing_start(client: Client, message: Message):
    ...
    try:
        ...
        # Сохраняем чаты в БД
        db.save_chats(user_id, active_account['id'], chats)
        ...
        # Показываем первый чат для выбора
        await show_next_chat_for_selection(client, message, user_id, active_account['id'])
...
async def show_next_chat_for_selection(client: Client, message: Message, user_id: int, account_id: int):
    """Показать следующий чат для выбора"""
    chats = db.get_chats(user_id, account_id)
    selected = db.get_selected_chats(user_id, account_id)
    
    if len(selected) >= MAX_CHATS_PER_MAILING:
        # Уже выбрали максимум чатов
        await ask_mailing_details(client, message, user_id, account_id)
        return
    
    # Ищем невыбранный чат
    next_chat = None
    for chat in chats:
    ...
    # Сохраняем текущий чат во временные данные
    temp_data = db.get_temp_data(user_id)
    temp_data['current_chat'] = next_chat
    db.save_temp_data(user_id, temp_data)
    
    keyboard = ReplyKeyboardMarkup(
        [
            [KeyboardButton("✅ Выбрать этот чат")],
    ...
    await message.reply_text(
        f"📢 *Чат {len(selected) + 1}/{MAX_CHATS_PER_MAILING}*\n\n"
        f"Название: {next_chat['chat_title']}\n"
        f"ID: {next_chat['chat_id']}\n\n"
        f"Выбрано чатов: {len(selected)}",
        reply_markup=keyboard
    )

async def ask_mailing_details(client: Client, message: Message, user_id: int, account_id: int):
    """Запросить детали рассылки"""
    selected_chats = db.get_selected_chats(user_id, account_id)
    ...
    chat_list = "\n".join([f"• {chat['chat_title']}" for chat in selected_chats])
...
@app.on_message(filters.text & filters.regex("^✅ Выбрать этот чат$"))
async def select_current_chat(client: Client, message: Message):
    user_id = message.from_user.id
    temp_data = db.get_temp_data(user_id)
    
    if 'current_chat' not in temp_data:
        await message.reply_text("❌ Ошибка. Начните заново.")
        return
    
    chat = temp_data['current_chat']
    ...
    # Выбираем чат
    db.select_chat(user_id, account_id, chat['chat_id'], True)
    
    # Показываем следующий
    await show_next_chat_for_selection(client, message, user_id, account_id)

@app.on_message(filters.text & filters.regex("^⏭ Пропустить$"))
async def skip_current_chat(client: Client, message: Message):
    user_id = message.from_user.id
    temp_data = db.get_temp_data(user_id)
    account_id = temp_data.get('account_id')
...
@app.on_message(filters.text & filters.regex("^✅ Завершить выбор$"))
...
@app.on_message(filters.text & ~filters.regex("^(📱 Менеджер аккаунтов|⚙️ Функции|👤 Профиль|➕ Добавить аккаунт|📋 Выбрать аккаунт|◀️ Назад в главное меню|◀️ Назад|❌ Отмена|📨 Рассылка|✅ Выбрать этот чат|⏭ Пропустить|✅ Завершить выбор)$"))
async def handle_text_input(client: Client, message: Message):
    ...
    if step == "waiting_api_id":
        ...
        if not text.isdigit():
            await message.reply_text("❌ API ID должен быть числом. Попробуйте снова:")
            return
        
        temp_data['api_id'] = text
        temp_data['step'] = "waiting_api_hash"
        db.save_temp_data(user_id, temp_data)
        
        await message.reply_text(
            "✅ API ID принят.\n\n"
            "✏️ Теперь введите API Hash:"
        )
    
    elif step == "waiting_api_hash":
        temp_data['api_hash'] = text
        temp_data['step'] = "waiting_phone"
        db.save_temp_data(user_id, temp_data)
        
        await message.reply_text(
            "✅ API Hash принят.\n\n"
            "📱 Теперь введите номер телефона в международном формате (например: +79001234567):"
        )
        ...
        try:
            ...
            await message.reply_text(
                "❌ Неверный API ID или API Hash. Проверьте данные и попробуйте снова.\n\n"
                "Нажмите 'Добавить аккаунт' для повторной попытки.",
                reply_markup=get_account_manager_keyboard()
            )
            db.clear_temp_data(user_id)
        ...
        if text.isdigit():
            ...
            if 0 <= index < len(accounts):
                ...
                await message.reply_text(
                    f"✅ Аккаунт {selected_account['account_name']} выбран активным.",
                    reply_markup=get_account_manager_keyboard()
                )
            else:
                await message.reply_text(
