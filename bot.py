...
def get_autosubscribe_keyboard():
    return ReplyKeyboardMarkup(
        [
...
def get_autoreply_chats_keyboard(chats: list, page: int = 0, total_pages: int = 1):
    ...
    for i in range(start_idx, end_idx):
        ...
        keyboard.append([KeyboardButton(f"{status} {chat['chat_title']}")])
...
def get_autosubscribe_chats_keyboard(chats: list, page: int = 0, total_pages: int = 1):
    keyboard = []
    start_idx = page * 5
    end_idx = min(start_idx + 5, len(chats))
...
class Database:
    ...
    def init_db(self):
        with self.get_connection() as conn:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS auto_subscribe_rules (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    user_id INTEGER,
                    account_id INTEGER,
                    chat_id TEXT,
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS auto_reply_rules (
                    ...
                    trigger_text TEXT,
                    reply_text TEXT,
                    match_type TEXT DEFAULT 'contains', -- 'contains', 'exact', 'startswith', 'regex'
                    is_active BOOLEAN DEFAULT 1,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            ''')
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                """INSERT INTO active_mailings 
                   (user_id, account_id, messages, chats, send_mode, message_count_per_chat, delay) 
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (user_id, account_id, json.dumps(messages), json.dumps(chats), 
                 send_mode, message_count_per_chat, delay)
            )
        ...
        with self.get_connection() as conn:
            ...
            if row:
                return {
                    ...
                    "delay": row[7],
                    "status": row[8],
                    "current_chat_index": row[9],
                    "current_message_index": row[10],
                    "sent_count": row[11],
                    "created_at": row[12]
                }
            return None
    ...
    # Методы для автоподписки
    def add_auto_subscribe_rule(self, user_id: int, account_id: int, chat_id: str, chat_title: str):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            # Проверяем, не существует ли уже такое правило
            cursor.execute(
                "SELECT id FROM auto_subscribe_rules WHERE user_id = ? AND account_id = ? AND chat_id = ?",
                (user_id, account_id, chat_id)
            )
            if not cursor.fetchone():
                cursor.execute(
                    """INSERT INTO auto_subscribe_rules 
                       (user_id, account_id, chat_id, chat_title) 
                       VALUES (?, ?, ?, ?)""",
                    (user_id, account_id, chat_id, chat_title)
                )
                conn.commit()
    ...
    def get_auto_subscribe_rules(self, user_id: int, account_id: Optional[int] = None) -> List[Dict]:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            if account_id:
                cursor.execute(
                    "SELECT * FROM auto_subscribe_rules WHERE user_id = ? AND account_id = ? AND is_active = 1",
                    (user_id, account_id)
                )
            else:
                cursor.execute(
                    "SELECT * FROM auto_subscribe_rules WHERE user_id = ? AND is_active = 1",
                    (user_id,)
                )
            rows = cursor.fetchall()
            rules = []
    ...
    def get_auto_subscribe_rule_for_chat(self, user_id: int, account_id: int, chat_id: str) -> Optional[Dict]:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM auto_subscribe_rules WHERE user_id = ? AND account_id = ? AND chat_id = ? AND is_active = 1",
                (user_id, account_id, chat_id)
            )
            row = cursor.fetchone()
            if row:
    ...
    def delete_auto_subscribe_rule(self, rule_id: int, user_id: int):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM auto_subscribe_rules WHERE id = ? AND user_id = ?",
                (rule_id, user_id)
            )
            conn.commit()
    
    def toggle_auto_subscribe_rule(self, rule_id: int, user_id: int, active: bool):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE auto_subscribe_rules SET is_active = ? WHERE id = ? AND user_id = ?",
    ...
    def add_auto_reply_rule(self, user_id: int, account_id: int, chat_id: str, chat_title: str, 
                           trigger_text: str, reply_text: str, match_type: str = 'contains'):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """INSERT INTO auto_reply_rules 
                   (user_id, account_id, chat_id, chat_title, trigger_text, reply_text, match_type) 
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (user_id, account_id, chat_id, chat_title, trigger_text, reply_text, match_type)
            )
            conn.commit()
    ...
    def get_auto_reply_rules(self, user_id: int, account_id: Optional[int] = None) -> List[Dict]:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            if account_id:
                cursor.execute(
    ...
    def get_auto_reply_rules_for_chat(self, user_id: int, account_id: int, chat_id: str) -> List[Dict]:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def toggle_auto_reply_rule(self, rule_id: int, user_id: int, active: bool):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
...
# Функция для проверки упоминания аккаунта
def check_account_mention(text: str, account_name: str, account_username: str) -> bool:
    """Проверяет, упоминается ли аккаунт в тексте"""
    if not text:
        return False
    ...
    if account_username and f"@{account_username.lower()}" in text_lower:
        return True
    
    # Проверяем упоминание по имени аккаунта
    if account_name and account_name.lower() in text_lower:
        return True
    
    return False

# Функция для проверки триггера автоответчика
def check_trigger_match(text: str, trigger: str, match_type: str) -> bool:
    """Проверяет, соответствует ли текст триггеру"""
    ...
    if match_type == 'exact':
        ...
        return text_lower.startswith(trigger_lower)
...
# Обработчик входящих сообщений для автоответчика и автоподписки
@app.on_message(filters.create(lambda _, __, m: m.chat.type != ChatType.PRIVATE and not m.from_user.is_self))
async def message_handler(client: Client, message: Message):
    """Обрабатывает входящие сообщения для автоответчика и автоподписки"""
    ...
    for (user_id,) in users:
        ...
        for account in accounts:
            ...
            # Проверяем, есть ли правила для этого чата
            auto_subscribe_rule = db.get_auto_subscribe_rule_for_chat(user_id, account['id'], chat_id)
            auto_reply_rules = db.get_auto_reply_rules_for_chat(user_id, account['id'], chat_id)
            
            if not auto_subscribe_rule and not auto_reply_rules:
                continue
            ...
            # Проверяем упоминание аккаунта для автоподписки
            if auto_subscribe_rule and check_account_mention(message_text, account['account_name'], account.get('account_username', '')):
                try:
                            ...
                            for button in row:
                                ...
                                if button.url and ("t.me" in button.url or "telegram" in button.url):
                                    ...
                                    await client.send_message(
                                        user_id,
                                        f"🔍 Найдено упоминание аккаунта {account['account_name']} в чате {chat_title}\n"
                                        f"Ссылка для подписки: {button.url}"
                                    )
                                    
                                    # Открываем ссылку в браузере или просто уведомляем
                                    # В Telegram нельзя автоматически переходить по ссылкам
                                    
                                elif button.callback_data:
                                    # Пробуем нажать на callback кнопку
                                    try:
                                        ...
                                        await client.send_message(
                                            user_id,
                                            f"✅ Выполнена подписка по кнопке в чате {chat_title}"
                                        )
                                    except Exception as e:
                                        await client.send_message(
                                            user_id,
                                            f"❌ Ошибка при нажатии на кнопку: {str(e)}"
                                        )
                except Exception as e:
                    print(f"Ошибка автоподписки: {e}")
            
            # Проверяем триггеры для автоответчика
            for rule in auto_reply_rules:
                if check_trigger_match(message_text, rule['trigger_text'], rule['match_type']):
                    try:
                        # Отправляем автоответ
                        await user_client.send_message(chat_id, rule['reply_text'])
                        
                        # Уведомляем пользователя
                        await client.send_message(
                            user_id,
                            f"🤖 Автоответ в чате {chat_title}\n"
                            f"Триггер: {rule['trigger_text']}\n"
                            f"Ответ отправлен: {rule['reply_text'][:50]}..."
                        )
                        
                        # Не отвечаем на несколько триггеров в одном сообщении
                        break
...
@app.on_message(filters.command("done_trigger"))
async def done_entering_trigger(client: Client, message: Message):
    user_id = message.from_user.id
    temp_data = db.get_temp_data(user_id)
    
    if temp_data.get('step') != 'entering_autosubscribe_trigger':
        await message.reply_text("❌ Нечего завершать.")
        return
    
    trigger_text = temp_data.get('temp_trigger')
    
    if not trigger_text:
        await message.reply_text("❌ Вы не ввели текст триггера.")
        return
    
    # Сохраняем правило автоподписки
    account_id = temp_data.get('account_id')
    selected_chats = temp_data.get('selected_chats', [])
    
    for chat in selected_chats:
        db.add_auto_subscribe_rule(
            user_id,
            account_id,
            chat['chat_id'],
            chat['chat_title']
    ...
    await message.reply_text(
        f"✅ Правило автоподписки успешно создано для {len(selected_chats)} чатов!\n\n"
        f"Теперь при упоминании вашего аккаунта в этих чатах бот будет искать кнопки для подписки.",
        reply_markup=get_autosubscribe_keyboard()
    )
...
async def list_accounts(client: Client, message: Message):
    ...
    for acc in accounts:
        ...
        text += f"• {acc['account_name']}{username_info}: {acc['phone_number']} - {status}{block_status}\n"
...
@app.on_message(filters.text & filters.regex("^⚡ Автоподписка$"))
async def autosubscribe_menu(client: Client, message: Message):
    user_id = message.from_user.id
    active_account = db.get_active_account(user_id)
    ...
    await message.reply_text(
        "⚡ Меню автоподписки\n\n"
        "Здесь вы можете настроить автоматическую подписку.\n"
        "Бот будет отслеживать упоминания вашего аккаунта в выбранных чатах "
        "и при наличии кнопок в сообщении пытаться подписаться.\n\n"
        "Выберите действие:",
        reply_markup=get_autosubscribe_keyboard()
    )

@app.on_message(filters.text & filters.regex("^📝 Добавить правило$"))
async def add_autosubscribe_rule_start(client: Client, message: Message):
    user_id = message.from_user.id
    active_account = db.get_active_account(user_id)
    ...
    db.save_temp_data(user_id, {
        "step": "selecting_autosubscribe_chats",
    ...
    await show_autosubscribe_chats_page(client, message, user_id, 0, total_pages)

async def show_autosubscribe_chats_page(client: Client, message: Message, user_id: int, page: int, total_pages: int):
    temp_data = db.get_temp_data(user_id)
    chats = temp_data.get('chats', [])
    
    await message.reply_text(
        f"📋 Выберите чаты для автоподписки (страница {page + 1}/{total_pages}):\n\n"
        "Нажимайте на кнопки с чатами, чтобы выбрать/отменить выбор.\n"
        "Когда закончите, нажмите '✅ Сохранить и продолжить'",
        reply_markup=get_autosubscribe_chats_keyboard(chats, page, total_pages)
    )

@app.on_message(filters.text & filters.regex("^📋 Список правил$") & filters.create(lambda _, __, m: db.get_temp_data(m.from_user.id).get('step') is None))
async def list_autosubscribe_rules(client: Client, message: Message):
    user_id = message.from_user.id
    active_account = db.get_active_account(user_id)
    ...
    rules = db.get_auto_subscribe_rules(user_id, active_account['id'])
    
    if not rules:
        await message.reply_text(
            "📋 У вас нет настроенных правил автоподписки.",
            reply_markup=get_autosubscribe_keyboard()
        )
        return
    
    text = "📋 Список правил автоподписки:\n\n"
    for i, rule in enumerate(rules, 1):
        text += f"{i}. Чат: {rule['chat_title']}\n"
        text += f"   ID: {rule['id']}\n\n"
...
async def delete_autosubscribe_rule_start(client: Client, message: Message):
    user_id = message.from_user.id
    active_account = db.get_active_account(user_id)
    ...
    db.save_temp_data(user_id, {
        "step": "deleting_autosubscribe_rule",
        "rules": rules
    })
...
async def list_autoreply_rules(client: Client, message: Message):
    user_id = message.from_user.id
    active_account = db.get_active_account(user_id)
    ...
    rules = db.get_auto_reply_rules(user_id, active_account['id'])
    ...
    for i, rule in enumerate(rules, 1):
        text += f"{i}. Чат: {rule['chat_title']}\n"
        text += f"   Триггер: {rule['trigger_text']}\n"
        text += f"   Ответ: {rule['reply_text'][:50]}...\n"
        text += f"   ID: {rule['id']}\n\n"
...
async def delete_autoreply_start(client: Client, message: Message):
    ...
    # Сохраняем список правил
    db.save_temp_data(user_id, {
        "step": "deleting_autoreply_rule",
        "rules": rules
    })
    
    text = "❌ Выберите автоответ для удаления:\n\n"
    for i, rule in enumerate(rules, 1):
        text += f"{i}. Чат: {rule['chat_title']} - {rule['trigger_text']}\n"
...
async def profile_menu(client: Client, message: Message):
    ...
    if active_account:
        ...
        text += f"• {active_account['account_name']}{username_info}\n"
...
# Обработчики для выбора чатов
@app.on_message(filters.text & filters.regex("^[⬜✅] .*$") & filters.create(lambda _, __, m: db.get_temp_data(m.from_user.id).get('step') in ['selecting_autoreply_chats', 'selecting_autosubscribe_chats']))
async def handle_chat_selection(client: Client, message: Message):
    user_id = message.from_user.id
    temp_data = db.get_temp_data(user_id)
    ...
    for chat in temp_data['chats']:
        if chat['chat_title'] == chat_title:
            ...
            if temp_data['step'] == 'selecting_autoreply_chats':
                await message.reply_text(
                ...
                await message.reply_text(
                    f"{'✅' if chat['selected'] else '⬜'} Чат '{chat_title}' {'выбран' if chat['selected'] else 'отменен'}",
                    reply_markup=get_autosubscribe_chats_keyboard(temp_data['chats'], page, total_pages)
                )
            return

@app.on_message(filters.text & filters.regex("^⬅️ Назад$") & filters.create(lambda _, __, m: db.get_temp_data(m.from_user.id).get('step') in ['selecting_autoreply_chats', 'selecting_autosubscribe_chats']))
async def prev_page(client: Client, message: Message):
    user_id = message.from_user.id
    temp_data = db.get_temp_data(user_id)
    ...
    if current_page > 0:
        ...
        if temp_data['step'] == 'selecting_autoreply_chats':
            ...
            await show_autosubscribe_chats_page(client, message, user_id, current_page - 1, total_pages)

@app.on_message(filters.text & filters.regex("^➡️ Вперед$") & filters.create(lambda _, __, m: db.get_temp_data(m.from_user.id).get('step') in ['selecting_autoreply_chats', 'selecting_autosubscribe_chats']))
async def next_page(client: Client, message: Message):
    user_id = message.from_user.id
    temp_data = db.get_temp_data(user_id)
...
@app.on_message(filters.text & filters.regex("^✅ Сохранить и продолжить$") & filters.create(lambda _, __, m: db.get_temp_data(m.from_user.id).get('step') in ['selecting_autoreply_chats', 'selecting_autosubscribe_chats']))
async def save_chats_and_continue(client: Client, message: Message):
    user_id = message.from_user.id
    temp_data = db.get_temp_data(user_id)
    ...
    if temp_data['step'] == 'selecting_autoreply_chats':
        temp_data['step'] = 'entering_trigger'
        db.save_temp_data(user_id, temp_data)
        
        await message.reply_text(
            f"✅ Выбрано чатов: {len(selected_chats)}\n\n"
            "📝 Введите текст-триггер для автоответа.\n"
            "На это сообщение будет срабатывать автоответчик.\n"
            "Вы также можете выбрать тип совпадения:\n"
            "1 - точное совпадение\n"
            "2 - начинается с текста\n"
            "3 - содержит текст\n"
            "4 - регулярное выражение\n\n"
            "Отправьте текст триггера или номер типа совпадения (1-4):",
            reply_markup=get_cancel_keyboard()
        )
    else:  # selecting_autosubscribe_chats
        # Для автоподписки не нужен дополнительный текст, просто сохраняем правила
        account_id = temp_data.get('account_id')
...
@app.on_message(filters.text & filters.create(lambda _, __, m: db.get_temp_data(m.from_user.id).get('step') in ['deleting_autoreply_rule', 'deleting_autosubscribe_rule']))
async def handle_delete_rule(client: Client, message: Message):
    user_id = message.from_user.id
    text = message.text
    temp_data = db.get_temp_data(user_id)
    ...
    if text.isdigit():
        ...
        if 0 <= index < len(rules):
            ...
            if temp_data['step'] == 'deleting_autoreply_rule':
                db.delete_auto_reply_rule(rule_to_delete['id'], user_id)
                rule_type = "автоответ"
            else:
                db.delete_auto_subscribe_rule(rule_to_delete['id'], user_id)
                rule_type = "правило автоподписки"
            
            db.clear_temp_data(user_id)
            
            await message.reply_text(
                f"✅ {rule_type} для чата '{rule_to_delete['chat_title']}' удален.",
                reply_markup=get_autoreply_keyboard() if temp_data['step'] == 'deleting_autoreply_rule' else get_autosubscribe_keyboard()
            )
        else:
...
# Обработчик для ввода триггера автоответа
@app.on_message(filters.text & filters.create(lambda _, __, m: db.get_temp_data(m.from_user.id).get('step') == 'entering_trigger'))
async def handle_trigger_input(client: Client, message: Message):
    user_id = message.from_user.id
    text = message.text
    temp_data = db.get_temp_data(user_id)
    
    if text in ["1", "2", "3", "4"]:
        # Сохраняем тип совпадения
        match_types = {
            "1": "exact",
            "2": "startswith",
            "3": "contains",
        ...
        temp_data['match_type'] = match_types[text]
        db.save_temp_data(user_id, temp_data)
        
        await message.reply_text(
            f"✅ Выбран тип совпадения: {text}\n\n"
            "✏️ Теперь введите текст триггера:"
        )
    else:
        # Сохраняем текст триггера и переходим к вводу ответа
        temp_data['trigger_text'] = text
        temp_data['step'] = 'entering_reply_text'
        db.save_temp_data(user_id, temp_data)
        
        await message.reply_text(
            f"✅ Триггер сохранен: {text[:50]}...\n\n"
            "✏️ Теперь введите текст для автоответа:"
        )
...
async def handle_reply_text_input(client: Client, message: Message):
    ...
    # Сохраняем правило автоответа
    account_id = temp_data.get('account_id')
    selected_chats = temp_data.get('selected_chats', [])
    trigger_text = temp_data.get('trigger_text')
    match_type = temp_data.get('match_type', 'contains')
...
@app.on_message(filters.text & ~filters.regex("^(📱 Менеджер аккаунтов|⚙️ Функции|👤 Профиль|➕ Добавить аккаунт|📋 Список аккаунтов|🔑 Выбрать активный аккаунт|❌ Удалить аккаунт|◀️ Назад в главное меню|◀️ Назад|◀️ Назад в функции|❌ Отмена|📨 Рассылка|🤖 Проверка спамблока|🔄 Автоответчик|⚡ Автоподписка|📝 Установить автоответ|📋 Список автоответов|❌ Удалить автоответ|📝 Добавить правило|📋 Список правил|❌ Удалить правило|📥 Загрузить ещё чаты|✅ Завершить выбор и продолжить|📝 Ввести сообщения|✅ Запустить рассылку|⬅️ Назад|➡️ Вперед|✅ Сохранить и продолжить|/done|/done_keywords|/done_trigger)$") & ~filters.regex("^🔄 Режим:.*$") & ~filters.regex("^[⬜✅] .*$") & ~filters.regex("^✅ Начать проверку$") & ~filters.regex("^[1-4]$"))
...
async def execute_mailing(client: Client, message: Message, user_id: int, temp_data: dict):
    ...
    if send_mode == 'sequential':
        await sequential_mailing(client, user_id, user_client, mailing_id, 
                                 messages, chats, message_count, delay)
    else:
